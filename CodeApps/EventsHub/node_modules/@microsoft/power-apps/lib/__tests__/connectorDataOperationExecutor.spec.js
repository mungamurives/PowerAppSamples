...
describe('ConnectorDataOperationExecutor', () => {
    ...
    describe('_buildOperationBodyParam', () => {
        it('should use the body parameter value directly without wrapping', async () => {
            // Arrange
            ...
            // Assert - body value should be used directly, not wrapped
            expect(JSON.parse(result)).toEqual({
                a: 1,
                b: 2,
            });
        });
        it('should use body parameter value directly regardless of parameter name', async () => {
            // Arrange
            const testInstance = new TestableConnectorDataOperation(clientProvider, connectionsService);
            const testOperationName = 'CreateWorkItem';
            const testTableName = 'AzureDevOps';
            const apiParams = [
                { name: 'connectionId', in: 'path', required: true },
                { name: 'account', in: 'query', required: true },
                { name: 'workItem', in: 'body', required: true },
            ];
            const testDataSourceInfo = {
                tableId: 'tid',
                version: 'v1',
            ...
            const parameters = {
                connectionId: '123',
                account: 'myorg',
                workItem: { title: 'My Task', description: 'Task description' },
            };
            const operation = {
                connectorOperation: {
                    tableName: testTableName,
            ...
            // Act
            const result = await testInstance.buildOperationBodyParam(operation, testTableName);
            // Assert - workItem value should be used directly, NOT wrapped as { workItem: {...} }
            expect(JSON.parse(result)).toEqual({
                title: 'My Task',
                description: 'Task description',
            });
        });
        it('should use body parameter named "parameters" directly for Azure DevOps HttpRequest', async () => {
            // Arrange - simulates Azure DevOps HttpRequest endpoint
            const testInstance = new TestableConnectorDataOperation(clientProvider, connectionsService);
            const testOperationName = 'HttpRequest';
            const testTableName = 'AzureDevOps';
            const apiParams = [
                { name: 'connectionId', in: 'path', required: true },
                { name: 'account', in: 'query', required: true },
            ...
            const parameters = {
                connectionId: '123',
                account: 'msazure',
                parameters: { Method: 'GET', Uri: '/_apis/projects?api-version=7.0' },
            };
            const operation = {
                connectorOperation: {
                    tableName: testTableName,
            ...
            // Act
            const result = await testInstance.buildOperationBodyParam(operation, testTableName);
            // Assert - parameters value should be used directly, NOT wrapped as { parameters: {...} }
            expect(JSON.parse(result)).toEqual({
                Method: 'GET',
